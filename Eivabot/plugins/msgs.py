...
async def all_groups_id(Eiva):
    Eivagroups = []
    async for dialog in Eiva.client.iter_dialogs():
        entity = dialog.entity
        if isinstance(entity, Channel) and entity.megagroup:
            Eivagroups.append(entity.id)
    return Eivagroups


@bot.on(Eiva_cmd(pattern="frwd$"))
...
@bot.on(Eiva_cmd(pattern="resend$"))
...
@bot.on(Eiva_cmd(pattern=r"fpost (.*)"))
