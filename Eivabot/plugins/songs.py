...
@bot.on(Eiva_cmd(pattern="lyrics(?: |$)(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="lyrics(?: |$)(.*)", allow_sudo=True))
async def nope(kraken):
    Eiva = kraken.pattern_match.group(1)
    await eor(kraken, f"Searching lyrics for  `{Eiva}` ...")
    if not Eiva:
        if kraken.is_reply:
    ...
    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(Eiva))}")
...
@bot.on(Eiva_cmd(pattern="song(?: |$)(.*)", outgoing=True))
...
@bot.on(Eiva_cmd(pattern="vsong(?: |$)(.*)", outgoing=True))
