...
from Eivabot.sql import blacklist_sql as sq
from . import *
...
@bot.on(Eiva_cmd(pattern="addblacklist ((.|\n)*)"))
...
@bot.on(Eiva_cmd(pattern="rmblacklist ((.|\n)*)"))
...
@bot.on(Eiva_cmd(pattern="listblacklist$"))
...
CmdHelp("blacklist").add_command(
  "addblacklist", "<word>/<words>", "The given word or words will be added to blacklist in that specific chat if any user sends then the message gets deleted.\n\nNote :- If you are adding more than one word at time via this, then remember that new word must be given in a new line that is not [hi Eivao]. It must be as [hi \n Eivao]"
).add_command(
