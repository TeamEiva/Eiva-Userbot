...
@bot.on(Eiva_cmd(pattern=r"cmds"))
...
async def kk(event):
    ...
    if len(OUTPUT) > 69:
        ...
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            ...
            await edit_or_reply(Eiva_file, f"Output Too Large. This is the file for the list of plugins in bot.\n\n**BY :-** {Eiva_USER}")
            await event.delete()


@bot.on(Eiva_cmd(pattern=r"send (?P<shortname>\w+)", outgoing=True))
...
@bot.on(Eiva_cmd(pattern="install$", outgoing=True))
    ...
    if event.reply_to_msg_id:
        try:
            downloaded_file_name = await event.client.download_media(  # pylint:disable=E0602
                ...
                "./Eivabot/plugins/"  # pylint:disable=E0602
            )
...
@bot.on(Eiva_cmd(pattern=r"uninstall (?P<shortname>\w+)", outgoing=True))
...
async def uninstall(kraken):
    ...
    dir_path =f"./Eivabot/plugins/{shortname}.py"
    try:
...
@bot.on(Eiva_cmd(pattern=r"unload (?P<shortname>\w+)$"))
...
@bot.on(Eiva_cmd(pattern=r"load (?P<shortname>\w+)$"))
...
CmdHelp("core").add_command(
  "install", "<reply to a .py file>", "Installs the replied python file if suitable to єιναϐοτ's codes."
).add_command(
  "uninstall", "<plugin name>", "Uninstalls the given plugin from єιναϐοτ. To get that again do .restart", "uninstall alive"
).add_command(
  ...
  "cmds", None, "Gives out the list of modules in EivaBot."
).add_warning(
