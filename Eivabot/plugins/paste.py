...
@bot.on(Eiva_cmd(pattern="paste ?(.*)", outgoing=True))
...
async def get_dogbin_content(dog_url):
    ...
    Eiva = await eor(dog_url, "`Getting dogbin content...`")
    ...
    if message.startswith(format_view):
        ...
        await eod(Eiva, "`Is that even a dogbin url?`")
        return
    ...
    try:
        ...
        await eod(Eiva, "Request returned an unsuccessful status code.\n\n" + str(HTTPErr)
        )
        return
    except exceptions.Timeout as TimeoutErr:
        await eod(Eiva, "Request timed out." + str(TimeoutErr))
        return
    except exceptions.TooManyRedirects as RedirectsErr:
        await eod(Eiva, "Request exceeded the configured number of maximum redirections."
            + str(RedirectsErr)
        )
        return
...
@bot.on(Eiva_cmd(pattern="neko ?(.*)", outgoing=True))
