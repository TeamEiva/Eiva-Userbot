...
from Eivabot.helpers import runner
from . import *
...
@bot.on(Eiva_cmd(outgoing=True, pattern=r"update(| now)$"))
...
@bot.on(Eiva_cmd(outgoing=True, pattern=r"update build$"))
...
async def upstream(event):
    ...
    off_repo = "https://github.com/TeamEiva/EivaBot"
    os.chdir("/app")
    git_Eiva = f"rm -rf .git"
    try:
        await runner.runcmd(git_Eiva)
    except BaseException:
        pass
    try:
        txt = "😕 `Updater cannot continue due to some problems occured`\n\n**LOGTRACE:**\n"
