...
@bot.on(Eiva_cmd("kickme", outgoing=True))
async def leave(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("😪 **KThnxBye** See u all in Eiva!!")
        time.sleep(1)
        if "-" in str(e.chat_id):
...
@bot.on(Eiva_cmd(pattern=r"config"))
...
@bot.on(Eiva_cmd(pattern="schd ?(.*)", outgoing=True))
...
CmdHelp("bot").add_command(
    ...
    "schd", "<secs> - <message>", "Sends your message in given secs", "schd 10 - Eivao"
).add_command(
