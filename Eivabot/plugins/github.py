...
@bot.on(Eiva_cmd(pattern=r"commit"))
...
async def download(event):
    ...
    if Config.GIT_REPO_NAME is None:
        await eod(event, "`Please ADD Proper Github Repo Name of EivaBot`")
        return
    Eivabot = await eor(event, "Processing ...")
    ...
    try:
        ...
        await Eivabot.edit("Committing to Github....")
        await git_commit(downloaded_file_name, Eivabot)


async def git_commit(file_name, Eivabot):
    content_list = []
    ...
    if create_file:
        ...
        try:
            ...
            await eod(Eivabot, "Cannot Upload Plugin")
    else:
        return await eod(Eivabot, "`Committed Suicide`")


@bot.on(Eiva_cmd(pattern="github (.*)", outgoing=True))
