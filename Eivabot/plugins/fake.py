...
@bot.on(Eiva_cmd(pattern="picgen"))
...
async def _(event):
    ...
    if response.status_code == 200:
      with open("EivaBOT.jpg", 'wb') as f:
        f.write(response.content)
    
    captin = f"Fake Image By {Eiva_mention}"
    fole = "EivaBOT.jpg"
    await bot.send_file(event.chat_id, fole, caption=captin)
    await event.delete()
    os.system("rm /root/Eivabot/EivaBOT.jpg ")


@bot.on(Eiva_cmd(pattern="fake ?(.*)"))
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 1320929227:
            await reply_message.reply(
                "`Wait a second, This is my master!`\n**How dare you threaten to ban my master nigger!**\n\n__Your account has been hacked! Pay 69$ to my master__ [Shivansh](https://t.me/Royal_King7) __to release your account__😏"
            )
        else:
            jnl = (
                "`Warning!! `"
