...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    Eivaevent = await eor(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eod(Eivaevent, "`Please unblock `@DrWebBot `and try again`")
            return
...
@bot.on(Eiva_cmd(pattern=r"decode$", outgoing=True))
...
@bot.on(Eiva_cmd(pattern="barcode ?(.*)"))
@bot.on(sudo_cmd(pattern="barcode ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Eivaevent = await eor(event, "...")
    ...
    try:
        ...
        await Eivaevent.edit(str(e))
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await Eivaevent.edit("Created BarCode in {} seconds🤓".format(ms))
    await asyncio.sleep(5)
    await Eivaevent.delete()


@bot.on(Eiva_cmd(pattern=r"makeqr(?: |$)([\s\S]*)", outgoing=True))
...
async def _(event):
    ...
    if len(input_sgra) == 3:
        ...
        await Eiva.edit(str(a))
    else:
        await eod(Eiva, f"SYNTAX: {hl}calendar YYYY.MM.DD")
    end = datetime.datetime.now()
    (end - start).seconds


@bot.on(Eiva_cmd(pattern="currency (.*)"))
...
@bot.on(Eiva_cmd(pattern="currencies$"))
...
@bot.on(Eiva_cmd(pattern="xkcd ?(.*)"))
@bot.on(sudo_cmd(pattern="xkcd ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Eivaevent = await eor(event, "`processiong...`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await Eivaevent.edit(output_str, link_preview=True)
    else:
        await eod(Eivaevent, "xkcd n.{} not found!".format(xkcd_id))

@bot.on(Eiva_cmd(pattern="dns (.*)", outgoing=True))
...
@bot.on(Eiva_cmd(pattern="url (.*)", outgoing=True))
...
@bot.on(Eiva_cmd(pattern="unshort (.*)", outgoing=True))
...
CmdHelp("tools").add_command(
  ...
  "currency", "<amount> <from> <to>", "Currency converter for EivaBot", ".currency 10 usd inr"
).add_command(
