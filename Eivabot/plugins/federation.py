...
@bot.on(Eiva_cmd(pattern="newfed ?(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="newfed ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Eiva_input = event.pattern_match.group(1)
    chat = "@MissRose_Bot"
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            await event.client.send_message(chat, f"/newfed {Eiva_input}")
...
@bot.on(Eiva_cmd(pattern="renamefed ?(.*)"))
...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
          try:     
              ...
              await event.client.send_message(chat, f"/renamefed {Eiva_input}")
              response = await response 
          except YouBlockedUserError: 
...
@bot.on(Eiva_cmd(pattern="fstat ?(.*)"))
@bot.on(sudo_cmd(pattern="fstat ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Eiva = await eor(event, "`Collecting fstat....`")
    thumb = Eiva_logo
    ...
    if lavde == "":
        ...
        async with bot.conversation(fbot) as conv:
            try:
                ...
                await Eiva.edit("`Please Unblock` @MissRose_Bot")


@bot.on(Eiva_cmd(pattern="fedinfo ?(.*)"))
@bot.on(sudo_cmd(pattern="fedinfo ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Eiva = await eor(event, "`Fetching fed info.... please wait`")
    lavde = event.pattern_match.group(1)
    async with bot.conversation(fbot) as conv:
        try:
            ...
            await Eiva.edit(massive.text + "\n\n**ʟɛɢɛռɖaʀʏ_ᴀғ_ΣIVΛBθƬ**")
        except YouBlockedUserError:
            await Eiva.edit("`Please Unblock` @MissRose_Bot")
