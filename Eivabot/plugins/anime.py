...
@bot.on(Eiva_cmd(pattern="anime(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern="anime(?: |$)(.*)", allow_sudo=True))
async def nope(hel_):
    Eiva = hel_.pattern_match.group(1)
    if not Eiva:
        if hel_.is_reply:
    ...
    troll = await bot.inline_query("AniFluidbot", f".anime {(deEmojify(Eiva))}")
...
async def nope(hel_):
    ...
    troll = await bot.inline_query("AniFluidbot", f".manga {(deEmojify(Eiva))}")
...
@bot.on(Eiva_cmd(pattern="character(?: |$)(.*)"))
...
async def nope(hel_):
    ...
    troll = await bot.inline_query("AniFluidbot", f".character {(deEmojify(Eiva))}")
