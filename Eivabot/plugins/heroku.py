...
async def restart(event):
    if HEROKU_APP_NAME and HEROKU_API_KEY:
        ...
        execl(executable, executable, "bash", "EivaBot")


@bot.on(Eiva_cmd(pattern="restart$"))
@bot.on(sudo_cmd(pattern="restart$", allow_sudo=True))
async def re(Eiva):
    if Eiva.fwd_from:
        return
    event = await eor(Eiva, "Restarting Dynos ...")
    if HEROKU_API_KEY:
        await restart(event)
    else:
...
@bot.on(Eiva_cmd(pattern="(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)", outgoing=True))
@bot.on(sudo_cmd(pattern="(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)", allow_sudo=True))
async def variable(Eiva):
    if Eiva.fwd_from:
        return
    if Config.HEROKU_APP_NAME is not None:
        app = Heroku.app(Config.HEROKU_APP_NAME)
    else:
        return await eor(Eiva, "`[HEROKU]:" "\nPlease setup your` **HEROKU_APP_NAME**")
    exe = Eiva.pattern_match.group(1)
    heroku_var = app.config()
    if exe == "get":
        event = await eor(Eiva, "Getting Variable Info...")
        await asyncio.sleep(1.5)
        cap = "Logger me chala jaa bsdk."
        capn = "Saved in LOGGER_ID !!"
        try:
            variable = Eiva.pattern_match.group(2).split()[0]
            if variable in ("EivaBOT_SESSION", "BOT_TOKEN", "HEROKU_API_KEY"):
                if Config.ABUSE == "ON":
        ...
        event = await eor(Eiva, "Setting Heroku Variable...")
        variable = Eiva.pattern_match.group(2)
        if not variable:
            return await event.edit(f"`{hl}set var <Var Name> <Value>`")
        value = Eiva.pattern_match.group(3)
        if not value:
            variable = variable.split()[0]
            try:
                value = Eiva.pattern_match.group(2).split()[1]
            except IndexError:
                return await event.edit(f"`{hl}set var <Var Name> <Value>`")
        await asyncio.sleep(1.5)
        if variable in heroku_var:
        ...
        event = await eor(Eiva, "Getting info to delete Variable")
        try:
            variable = Eiva.pattern_match.group(2).split()[0]
        except IndexError:
...
async def dyno_usage(Eiva):
    if Eiva.fwd_from:
        return
    event = await edit_or_reply(Eiva, "`Processing...`")
    useragent = (
...
@bot.on(Eiva_cmd(pattern="logs$"))
@bot.on(sudo_cmd(pattern="logs$", allow_sudo=True))
async def _(dyno):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):
        return await eor(dyno, f"Make Sure Your HEROKU_APP_NAME & HEROKU_API_KEY are filled correct. Visit {Eiva_grp} for help.", link_preview=False)
    try:
