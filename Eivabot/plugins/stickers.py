...
Eivabot = Config.STICKER_PACKNAME


@bot.on(Eiva_cmd(outgoing=True, pattern="kang"))
...
async def kang(args):
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            Eiva = await eor(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
    ...
    if photo:
        ...
        packname = f"{user.username}_Eivabot_{pack}"
        packnick = (
            f"{Eivabot} Vol.{pack}"
            if Eivabot
            else f"@{user.username}'s EivaBot Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
        ...
        if (
            ...
            await Eiva.edit("`Preparing a new pack....`")
            async with bot.conversation("Stickers") as conv:
                await conv.send_message(cmd)
                await conv.get_response()
                # Ensure user doesn't get spamming notifications
...
@bot.on(Eiva_cmd(pattern=r"delst ?(.*)", outgoing=True))
...
@bot.on(Eiva_cmd(pattern=r"editst ?(.*)", outgoing=True))
    ...
    if Eiva == "":
        ...
        async with bot.conversation(chat) as conv:
            try:
                ...
                await conv.get_response()
                await asyncio.sleep(2)
                await conv.send_message(f"{Eiva}")
                response = await response
            except YouBlockedUserError:
                await event.reply("Please unblock @Stickers and try again")
                return
...
async def sticklet(event):
    ...
    FONT_FILE = await get_font_file(event.client, "@EivaFonts")
    ...
    image_stream.name = "Eivabot.webp"
...
@bot.on(Eiva_cmd(pattern="waifu(?: |$)(.*)", outgoing=True))
