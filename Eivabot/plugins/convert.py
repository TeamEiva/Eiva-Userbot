...
@bot.on(Eiva_cmd(pattern="stoi$"))
@bot.on(sudo_cmd(pattern="stoi$", allow_sudo=True))
async def _(Eiva):
    if Eiva.fwd_from:
        return
    reply_to_id = Eiva.message.id
    if Eiva.reply_to_msg_id:
        reply_to_id = Eiva.reply_to_msg_id
    event = await eor(Eiva, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await Eiva.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await Eiva.client.send_file(
                event.chat_id,
...
@bot.on(Eiva_cmd(pattern="gif$"))
@bot.on(sudo_cmd(pattern="gif$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Eivareply = await event.get_reply_message()
    if not Eivareply or not Eivareply.media or not Eivareply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if Eivareply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    Eivaevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with bot.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await bot.send_file(chat, Eivareply.media)
            response = await conv.get_response()
            await bot.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await Eivaevent.edit("`This file is not supported`")
            Eivaresponse = response if response.media else await conv.get_response()
            await bot.send_read_acknowledge(conv.chat_id)
            Eivafile = Path(await event.client.download_media(Eivaresponse, "./temp/"))
            Eivagif = Path(await unzip(Eivafile))
            ...
            for files in (Eivagif, Eivafile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await Eivaevent.edit("Unblock @tgstogifbot")
            return


@bot.on(Eiva_cmd(pattern="nfc ?(.*)"))
