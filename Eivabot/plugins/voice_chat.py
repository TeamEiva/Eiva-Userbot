...
@bot.on(Eiva_cmd(pattern="startvc$"))
...
@bot.on(Eiva_cmd(pattern="vcinvite$"))
@bot.on(sudo_cmd(pattern="vcinvite$", allow_sudo=True))
async def _(event):
    Eiva = await eor(event, "`🧐 Inviting Users To Voice Chat....`")
    users = []
    i = 0
    ...
    for k in hel_:
        try:
            ...
            pass
    await Eiva.edit(f"**🚀 Invited {i} Users to Voice Chat**")
