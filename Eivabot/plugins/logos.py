...
async def lg1(Eivaevent):
    ...
    if Eivaevent.reply_to_msg_id:
        ...
        async for i in bot.iter_messages("@EivaBOT_LOGOS", filter=InputMessagesFilterPhotos):
    	    PICS_STR.append(i)
        pic = random.choice(PICS_STR)
        logo_ = await pic.download_media()
    text = Eivaevent.pattern_match.group(1)
    if len(text) <= 8:
        font_size_ = 150
        strik = 10
    elif len(text) >= 9:
    ...
    file_name = "EivaBot.png"
    img.save(file_name, "png")
    await bot.send_file(
        Eivaevent.chat_id,
        file_name,
        caption=f"**Made By :** {Eiva_mention}",
    )
    await event.delete()
    try:
        os.remove(file_name)
