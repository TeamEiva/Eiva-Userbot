...
from . import *
from Eivabot.sql.mute_sql import is_muted, mute, unmute
...
@bot.on(Eiva_cmd(pattern="setgpic$"))
...
@bot.on(Eiva_cmd(pattern="promote(?: |$)(.*)"))
...
async def promote(promt):
    ...
    Eivaevent = await eor(promt, "`Promoting User...`")
    ...
    try:
        ...
        await Eivaevent.edit(f"**🔥 Promoted  [{user.first_name}](tg://user?id={user.id})  Successfully In**  `{promt.chat.title}`!! \n**Admin Tag :**  `{rank}`")
    except BadRequestError:
        await Eivaevent.edit(NO_PERM)
        return
...
@bot.on(Eiva_cmd(pattern="demote(?: |$)(.*)"))
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    Eivaevent = await eor(dmod, "`Demoting User...`")
    rank = "ǟɖʍɨռ"
    ...
    try:
        ...
        await Eivaevent.edit(NO_PERM)
        return
    await Eivaevent.edit(f"**😪 Demoted  [{user.first_name}](tg://user?id={user.id})  Successfully In**  `{dmod.chat.title}`")
...
@bot.on(Eiva_cmd(pattern=r"mute ?(.*)"))
@bot.on(sudo_cmd(pattern=r"mute ?(.*)", allow_sudo=True))
async def muth(Eiva):
    if Eiva.is_private:
        await eor(Eiva, "**Enough of your bullshit  !!**")
        await sleep(2)
        await Eiva.get_reply_message()
        replied_user = await Eiva.client(GetFullUserRequest(Eiva.chat_id))
        if is_muted(Eiva.chat_id, Eiva.chat_id):
        ...
        try:
            ...
            await eor(Eiva, f"**Error **\n`{str(e)}`")
        else:
            await eor(Eiva, "**Chup Reh Lawde 🥴\n`**｀-´)⊃━☆ﾟ.*･｡ﾟ **`")
    else:
        Eivaevent = await eor(Eiva, "`Muting...`")
        input_str = Eiva.pattern_match.group(1)
        chat = await Eiva.get_chat()
        if Eiva.reply_to_msg_id:
            userid = (await Eiva.get_reply_message()).sender_id
            ...
            if input_str.isdigit():
                try:
                    ...
                    return await Eivaevent.edit(str(ve))
            else:
                userid = (await Eiva.client.get_entity(input_str)).id
                name = (await Eiva.client.get_entity(userid)).first_name
        else:
        ...
        try:
            await Eiva.client.edit_permissions(
                chat.id,
                userid,
                until_date=None,
                send_messages=False,
...
@bot.on(Eiva_cmd(pattern=r"unmute ?(.*)"))
...
async def nomuth(evn):
    if evn.is_private:
        ...
        Eivaevent = await eor(evn, "`Unmuting...`")
        ...
        if evn.reply_to_msg_id:
            ...
            return await eod(Eivaevent, "I need a user to unmute!!", 3)
        try:
...
@bot.on(Eiva_cmd(pattern="ban(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern="ban(?: |$)(.*)", allow_sudo=True))
@errors_handler
async def ban(bon):
    if bon.fwd_from:
        return
    Eivaevent = await eor(bon, "`Banning Nigga...`")
    ...
    if not user:
        return await Eivaevent.edit("`Reply to a user or give username!!`")
    if str(user.id) in DEVLIST:
        return await Eivaevent.edit("**Say again? Ban my creator??**")
    try:
    ...
    try:
        ...
        await Eivaevent.edit(f"**Banned  [{user.first_name}](tg://user?id={user.id})  in** `[{bon.chat.title}]` !!\n\nMessage Nuking : **False**")
        return
    if reason:
        await Eivaevent.edit(f"**Bitch** [{user.first_name}](tg://user?id={user.id}) **is now banned in**  `[{bon.chat.title}]` !!\n**Reason :** `{reason}`")
    else:
        await Eivaevent.edit(f"**Bitch** [{user.first_name}](tg://user?id={user.id}) **is now banned in**  `[{bon.chat.title}]`!!")
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    Eivaevent = await eor(unbon, "`Unbanning...`")
    ...
    try:
        ...
        await Eivaevent.edit(f"[{user.first_name}](tg://user?id={user.id}) **Is Now Unbanned in**  `{unbon.chat.title}` !!")
        ...
        await Eivaevent.edit("Invalid UserId!! Please Recheck it!!")


@bot.on(Eiva_cmd(pattern="pin($| (.*))"))
...
async def kick(usr):
    ...
    Eivaevent = await eor(usr, "`Kicking...`")
    try:
        await usr.client.kick_participant(usr.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await Eivaevent.edit(NO_PERM + f"\n`{str(e)}`")
        return
    if reason:
        await Eivaevent.edit(
            f"**🏃 Kicked**  [{user.first_name}](tg://user?id={user.id})'s **Butt from** `{usr.chat.title}!`\nReason: `{reason}`"
        )
    else:
        await Eivaevent.edit(f"**🏃 Kicked**  [{user.first_name}](tg://user?id={user.id})'s **Butt from** `{usr.chat.title}!`")
...
@bot.on(Eiva_cmd(pattern=f"zombies ?(.*)"))
